...
## 系统模板选择
准备安装 Dokploy 的第一步，是在腾讯云控制台重装系统或购买服务器时，直接在应用镜像（或系统镜像）里选择 **Docker CE** 镜像。

这样系统初始化后就自带了 Docker 环境，省去了我们手动安装 Docker 的麻烦，直接进入正题。
...
一套组合拳打下来，一个字：赞！再也不用担心国内服务器拉取镜像卡半天了。
